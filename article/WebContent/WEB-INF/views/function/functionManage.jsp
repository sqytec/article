	...
	<body>
						...
						<section class="vbox">
							<section class="scrollable padder">
								...
								<section class="panel panel-default">
									<header class="panel-heading">功能列表</header>
									
									<form action="<%= basePath %>fun/funs" method="post" 
											...
											<div class="col-sm-10 m-b-xs">
												<div class="inline" style="margin-right:20px;">
													...
													<input type="text" id="keyword" name="keyword" class="input-sm form-control inline" 
														style="width: 150px;" value="${funname}" placeholder="请输入功能名称"
														data-rangelength="[4,15]">
														
													<button class="btn btn-sm btn-default">查询</button>
												</div>
	
												<a href="<%= basePath %>fun/toAdd" class="btn btn-dark btn-sm">
											...
											<table class="table table-striped b-t b-light text-sm">
												...
												<thead>
													<tr>
														...
														<th style="text-align: center;">功能URL</th>
													...
													<c:choose>
														<c:when test="${!empty page.list}">
															<c:forEach items="${page.list}" var="fun">
																<tr>
																	<td style="line-height: 30px;text-align: center;">${fun.funname}</td>
																	<td style="line-height: 30px;">
																		<c:choose>
																			<c:when test="${fun.funurl != null && !empty fun.funurl}">${fun.funurl}</c:when>
																			<c:otherwise>无</c:otherwise>
																		</c:choose>
																	</td>
																	<td style="line-height: 30px;text-align: center;">${fun.parentFunction.funname}</td>
																	<td style="line-height: 30px;text-align: center;">
																		<c:choose>
																			<c:when test="${fun.funstate == 1}">正常</c:when>
																			<c:otherwise>禁用</c:otherwise>
																		</c:choose>
																	</td>
																	<td style="line-height: 30px;text-align: center;">
																		<a href="<%= basePath %>fun/toModify?funid=${fun.funid}" class="btn btn-warning btn-sm">编辑</a>
																		<c:choose>
																			<c:when test="${fun.funstate == 1}">
																				<a href="<%= basePath %>fun/disable?funid=${fun.funid}" class="btn btn-danger btn-sm"
																						onclick="return confirmDelete(this);">禁用</a>
																			</c:when>
																			<c:otherwise>
																				<a href="<%= basePath %>fun/enable?funid=${fun.funid}" class="btn btn-primary btn-sm">启用</a>
																			</c:otherwise>
																		</c:choose>
		...
		<script type="text/javascript">
			...
			function getRequestURL(page) {
				...
				var url = host + "fun/funs?pageNo=" + pageNo + "&pageCount=" + pageCount 
							+ "&keyword=" + keyword;
				return url;
			}
