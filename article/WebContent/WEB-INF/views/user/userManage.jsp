	...
	<body>
						...
						<section class="vbox">
							<section class="scrollable padder">
								...
								<section class="panel panel-default">
									...
									<form action="<%= basePath %>user/users" method="post" data-validate="parsley">
										<!-- 查询条件 -->
											...
											<div class="col-sm-10 m-b-xs">
												<div class="inline" style="margin-right:20px;">
													...
													<input type="text" id="keyword" name="keyword" class="input-sm form-control inline" 
														style="width: 150px;" placeholder="请输入用户真实姓名" value="${keyword}"
														data-rangelength="[4,30]">
														
													<button class="btn btn-sm btn-default">查询</button>
												</div>
	
												<a href="<%= basePath %>user/toAdd" class="btn btn-dark btn-sm">
													<i class="fa fa-plus"></i> 新增用户
												</a>
												
												<a href="<%= basePath %>user/toAddBatch" class="btn btn-primary btn-sm">
													<i class="fa fa-plus"></i> 批量导入用户
												</a>
											</div>
										</div>
													...
													<c:choose>
														<c:when test="${!empty page.list}">
															<c:forEach items="${page.list}" var="user">
																<tr>
																	<td style="line-height: 30px;text-align: center;">${user.usertruename }</td>
																	<td style="line-height: 30px;text-align: center;">${user.loginname}</td>
																	<td style="line-height: 30px;text-align: center;">${user.phone}</td>
																	<td style="line-height: 30px;text-align: center;">${user.institution.instname}</td>
		...
		<script type="text/javascript">
			...
			function getRequestURL(page) {
				...
				var url = host + "user/users?pageNo=" + pageNo + "&pageCount=" + pageCount 
							+ "&keyword=" + keyword;
				return url;
			}
